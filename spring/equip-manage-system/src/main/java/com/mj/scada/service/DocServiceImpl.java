...
public class DocServiceImpl implements DocService
{
    ...
    public ReturnData add(MultipartFile file, String docName, String type, String classify, String author, String introduction)
    {
        ...
        minioUtils.upload(file, docBucket);

        //将信息存入数据库
        if (docRepository.findByDocName(docName) == null)
        {
            Doc doc = new Doc(docName, type, classify, Util.getCurrentDateAndTime(), 0, author, introduction);
    ...
    public ReturnData detail(Integer id)
    {
        ...
        for (String s : list)
        {
            map.put(s, Util.equipNameMap.get(s));
        }
