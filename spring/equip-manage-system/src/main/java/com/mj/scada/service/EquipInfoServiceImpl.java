...
public class EquipInfoServiceImpl implements EquipInfoService
{
    ...
    {
        //设备列表
        Map<String, Map<String, String>> equipMap = JSON.parseObject(Util.getEquipList(restTemplate), Map.class);
        //获取map中的第一个元素
    ...
    {
        //设备名称
        String name = Util.equipNameMap.get(deviceNum);

        //设备开关机状态、故障状态
        Integer startup=Util.getEquipsStartup().get(deviceNum);
        Integer fault=Util.getEquipsFault().get(deviceNum);
        ...
        Integer startupTime = equipTimeTodayService.getTodayStartupTime(deviceNum);
        Integer closeTime = equipTimeTodayService.getTodayCloseTime(deviceNum);
    ...
    public ReturnData uploadImg(MultipartFile file, String deviceNum)
    {
        ...
        return minioUtils.upload(file, deviceNum);
    }
